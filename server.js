...
function initFiles() {
    ...
    if (fs.existsSync(CADASTRE_FILE)) cadastre = JSON.parse(fs.readFileSync(CADASTRE_FILE, 'utf8'));
    if (fs.existsSync(ALANSTORE_FILE)) alanStore = JSON.parse(fs.readFileSync(ALANSTORE_FILE, 'utf8'));
}
initFiles();
...
wss.on('connection', (ws) => {
    ...
    ws.on('message', (message) => {
        try {
            ...
            if (data.type === 'auth') {
                const { user, pass, isRegister } = data;
                if (!user || !pass || user.length < 3) return ws.send(JSON.stringify({ type: 'error', msg: 'Identifiants invalides.' }));
                
                const hashedPass = hashPassword(pass);

                if (isRegister) {
                    if (usersDb[user]) return ws.send(JSON.stringify({ type: 'error', msg: 'Pseudo déjà pris.' }));
                    ...
                    if (!usersDb[user] || usersDb[user].pass !== hashedPass) return ws.send(JSON.stringify({ type: 'error', msg: 'Mauvais mot de passe.' }));
                }
            ...
            if (data.type === 'interact') {
                ...
                // Distance Check (max 10 pixels)
                const dist = Math.hypot(playerState.x - tx, playerState.y - ty);
                if (dist > 15) return ws.send(JSON.stringify({ type: 'error', msg: 'Trop loin !' }));
                ...
                if (tx >= 475 && tx <= 525 && ty >= 475 && ty <= 525 && data.action === 'change_job') {
                    if (data.job) {
                        ...
                        dbRef.job = data.job;
                        ws.send(JSON.stringify({ type: 'sys', msg: `Métier changé pour : ${data.job}`}));
                    }
                    return;
                }

                // Vérification Stamina pour actions physiques
                if (dbRef.stamina < 1) return ws.send(JSON.stringify({ type: 'error', msg: 'Endurance épuisée. Reposez-vous ou mangez.' }));

                // Logique selon Métier
                if (playerState.job === 'ouvrier') {
                    if (data.action === 'mine' && targetId === 0) { // Mine Pixelium
                        pixelUpdates.set(`${tx},${ty}`, 20); // Devient de la terre beige/bois clair après minage (épuisé)
                        dbRef.stamina -= 1;
                        dbRef.inventory.push({ id: 'pixelium', qty: 1 }); // Simplifié
                        ws.send(JSON.stringify({ type: 'sys', msg: '+1 Pixelium' }));
                    }
                    if (targetId === 10 && data.action === 'craft') { // Etabli
                        // Logique de craft simplifiée (Pixelium -> Pix)
                    ...
                    if (data.action === 'build' && data.colorId !== undefined) {
                        if (!canModify(ws.user, tx, ty)) return ws.send(JSON.stringify({ type: 'error', msg: 'Terrain Privé.' }));
                    ...
                    if (data.action === 'shoot') {
                        dbRef.stamina -= 1;
                        // Raycast simplifé : touche le premier joueur proche de la cible
                        for (let [oid, ostate] of activePlayers.entries()) {
                            if (oid !== ws.id && Math.hypot(ostate.x - tx, ostate.y - ty) < 3) {
                                ...
                                if (usersDb[ostate.user].hp <= 0) {
                                    // Mort = Inventaire transféré à l'Alan Store (Occasion) et Respawn
    ...
    ws.on('close', () => {
        if (ws.user) {
            // Sécurité anti-fuite RAM : Si inventaire non sécurisé (pas dans un coffre), il va à l'Alan Store
            // Simplification : ici on assume qu'on sauvegarde toujours, mais dans la vraie logique Hardcore, 
            // on purgerait l'inventaire si le joueur n'est pas sur son terrain.
...
setInterval(() => {
    ...
    for (let [id, state] of activePlayers.entries()) {
        ...
        if (now - state.lastMove > 60000) { // Immobile 60s
    ...
    if (Math.random() < 0.1) {
        ...
        if (board[getIndex(rx, ry)] === 0) { // Pousse sur le blanc pur
            pixelUpdates.set(`${rx},${ry}`, 0); // Visuellement on pourrait ajouter une particule, mais le bloc reste 0. On envoie juste l'event.
        }
    }
...
<head>
    ...
    <style>
        ...
        .btn:hover { background: #2c3e50; border-color: #f39c12; }
        ...
        /* NOTIFICATIONS */
        #notif { position: absolute; top: 70px; left: 50%; transform: translateX(-50%); background: rgba(46, 204, 113, 0.9); padding: 10px 20px; border-radius: 20px; font-weight: bold; opacity: 0; transition: opacity 0.3s; pointer-events: none; }
...
<body>
    ...
    <div class="modal show" id="modal-login">
        ...
        <button class="btn" onclick="auth(false)">Se connecter</button>
        <button class="btn" onclick="auth(true)" style="background: #8e44ad;">Créer un compte</button>
    </div>

    <!-- MODAL NEXUS -->
    <div class="modal" id="modal-nexus">
        <h2>NEXUS - Bureau des Emplois</h2>
        <p>Choisissez votre voie. Coût: Gratuit (Beta).</p>
    ...
    <script>
        ...
        function initNet() {
            ...
            fetch('/board.dat').then(res => res.arrayBuffer()).then(buffer => {
                ...
                // Le serveur ne nous donne pas encore le dico de couleur, on attend la socket
        ...
        function connectSocket(rawView, imgData) {
            ...
            ws.onmessage = (e) => {
                ...
                if (data.type === 'auth_success') {
                    ...
                    myUser = document.getElementById('inp-user').value;
        ...
        canvas.addEventListener('mousedown', (e) => {
            ...
            let action = 'interact';
            if (myJob === 'ouvrier') action = 'mine'; // ou 'craft' si sur établi, géré côté serv
        ...
        function renderLoop(time) {
            ...
            // Effet Torique Visuel (dessine 9 fois la map autour pour illusion infinie)
            // Pour des raisons de perfs, dessiné uniquement si proche des bords, mais ici forcé :
